...
3. Click** Next** to set the duration of time that the endpoint waits for a response from the outbound member. Complete the following to specify session timeout value:
   ...
   - **Log** **Level**: From the drop-down list, select the severity level for the events. The severity level you select here determines the type of events that are being logged. This can be **Debug**, **Info**, **Warning**, or **Error**. When you select **Debug**, all fields or variables used in the events that were sent to the endpoint are logged, including deduplicated events for RPZ hits. For information about deduplication, see [*Deduplicating Events*](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280759890). Note that setting this to **Debug** might slightly affect the performance of your production system.
   - **Template**: Click **Select** **Template** to select a session management template. For information, see [*Creating Session Management Templates*](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280759926).
...
4. Click **Next** to add extensible attributes for the endpoint. For information, see [*Managing Extensible Attributes*](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280274215).
