...
4. In the *Add* *Response* *Policy* *Zone* *Wizard*, select **Add** **FireEye-Integrated** **Response** **Policy** **Zone**, click **Next** and specify the following:
   ...
   - **Disable:** Select the checkbox to disable the FireEye integrated RPZ without deleting its configuration. Clear the checkbox to enable the FireEye integrated RPZ. For information, see [*Enabling and Disabling Zones*](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280763205)*.* Note that disabling the FireEye integrated RPZ may take a longer time to complete depending on the size of the data.
...
| --- | --- | --- |
| Domain Match  Infection Events  Callback Events  Malware Object  Web Infection | Select a value from the drop-down list for a FireEye alert when a malware object is detected: **None**, **Passthru**, **Block** **(No** **Such** **Domain)**, **Block** **(No** **Data)**, and **Substitute** **(Domain** **Name)**. The drop-down list displays **Passthru**, by default. For more information about the **RPZ** **Policy** **Types**, see [*Configuring Rules for RPZs*](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280269891). | Specify a lifetime for each FireEye alert in **Days**, **Weeks**, or select **Live** **Forever** from the drop-down list. The following are the default values for different alert types:  - Domain Match - 1 week - Infection Events - 1 day - Callback Events - 1 week - Malware Object - 1 day - Web Infection - 1 day      Click on the default value to change the lifetime value. |
