...
> ### Note
> 
> When you make inline changes to an interface, a new task is created under Grid Manager, which you can view in the Task Manager page (for more information, see [*Viewing Tasks*](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280268354)). A status icon appears next to the interface element you have changed, indicating the status of the new task and providing a link to the Task Manager page. New tasks appear with a status icon of Pending (||). When the new task completes, the icon changes to a green checkmark.
