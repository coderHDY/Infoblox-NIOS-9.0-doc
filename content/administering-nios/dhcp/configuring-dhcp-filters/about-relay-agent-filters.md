...
pageId: "280761466"
---
The NIOS appliance can filter an address request by the circuit ID and remote ID of a requesting host. The filter instructs the appliance either to grant or deny an address request if the requesting host matches the filter. For information about the DHCP relay agent option, see as described in [About the DHCP Relay Agent Option (Option 82)](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280271370).  
...
2. Apply the filter to a DHCP address range or range template, and specify that if the circuit ID or remote ID of a requesting host matches the filter definition, the appliance either grants or denies the address assignment. For information, see *[Applying Filters to DHCP Objects](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280270743)**.*
3. Define the access privileges of limited-access admin group for relay agent filters. For information, see *[Managing Administrators](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280268091)*.
...
After you define a relay agent filter, you can assign it to a DHCP range. The appliance responds to address requests based on the filter criteria. For information, see *[Applying Filters to DHCP Objects](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280270743)*.
