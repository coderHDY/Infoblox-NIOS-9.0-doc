...
You can also create an option filter the appliance uses to filter address requests by the DHCP options of requesting hosts. The filter instructs the appliance to either grant or deny an address request if the requesting host matches the filter. For information, see [*Defining Option Filters*](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280403707).  
