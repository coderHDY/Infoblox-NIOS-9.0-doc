...
2. In the *Add* *Microsoft* *Server(s)* wizard, complete the following:
   ...
   - **Which** **features** **do** **you** **want** **to** **configure?**: This section appears only when you have selected the **Enable** **MS** **AD** **feature** checkbox for mapping network users. For more information, see [*Enabling Identity Mapping*](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280764411). You can select multiple options in this section:
   ...
   - See [*Viewing Synchronization Logs*](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280761116/Monitoring+Managed+Microsoft+Servers#ViewingSynchronizationLogs) for a description of each level.
   - **Logging** **output** **destination**: From the drop-down list, select an output destination to which the appliance saves log messages for Microsoft servers. When you select **Microsoft** **Log**, the appliance logs the messages that are generated for the respective Microsoft server in the existing Microsoft log. This is selected by default. For more information, see [*Viewing Synchronization Logs*](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280761116/Monitoring+Managed+Microsoft+Servers#ViewingSynchronizationLogs). When you select **Syslog**, NIOS logs the messages that are generated for the respective Microsoft server in the syslog. For more information about the syslog, see [*Viewing the Syslog*](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280403148/Using+a+Syslog+Server#ViewingtheSyslog).
...
5. Click **Next** and do the following in the Managed Servers table:
   ...
   - **DNS Monitor & Control**: Click **Override** to override the setting inherited from the Grid. To inherit the same settings as the Grid, click **Inherit**. Select this to enable monitoring and the ability to control DNS service for the Microsoft server. For more information, see [*Setting Grid Properties for Managing Microsoft Servers*](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280662859).
   - **Synchronize DNS Reporting Data**: Click **Override** to override the settings that are inherited from the Grid. To retain the same settings as the Grid, click **Inherit**. Select this to synchronize DNS reporting data from the Microsoft server. For more information, see [*Synchronizing DNS Reporting Data*](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280662859).  
   ...
   - **DHCP Monitor & Control**: Click **Override** to override the setting inherited from the Grid. To inherit the same settings as the Grid, click **Inherit**. Select this to monitor and control DHCP service for the Microsoft server. For more information, see [*Setting Grid Properties for Managing Microsoft Servers*](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280662859).
   ...
   - **Synchronize** **Network** **Users**: Click **Override** to override the settings inherited from the Grid. To inherit the same settings as the Grid, click **Inherit**. Select this to enable the identity mapping for the Microsoft server. For information, see [*Enabling Identity Mapping*](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280764411).  
...
Click **Next:** Continue to the next step and define extensible attributes for the Microsoft servers. For information, see [*Managing Extensible Attributes*](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280274215).
...
- View the status of the servers in the *Microsoft* *Servers* panel, as described in [*Monitoring Managed Microsoft Servers*](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280761116). Newly added servers first display a status of **Connecting** as the Grid member contacts the Microsoft servers. The status changes to **OK** after the Grid member successfully connects to the Microsoft server.
- View the data synchronized from the Microsoft servers. To view DNS data, navigate to the DNS view you specified. For information, see [*Viewing Zones*](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280405374). To view DHCP data, navigate to the **Networks** tab of the network view that you specified. For information, see [*Managing IPv4 DHCP Data*](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280267800).
...
- Use Smart Folders to organize the Microsoft servers and their data. For example, you can create a folder for DNS zones and another folder for DHCP scopes synchronized from a Microsoft server. For information about Smart Folders, see [*Smart Folders*](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280757969).
- Update the synchronized data. For information, see [Managing Microsoft DNS Services](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280401233)*,* and [*Managing Microsoft DHCP Services*](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280660915).

You can also use Global Search to search for synchronized data, such as zones and IP addresses. For information, see [*Using Global Search*](https://infoblox-docs.atlassian.net/wiki/spaces/nios90/pages/280401588).
