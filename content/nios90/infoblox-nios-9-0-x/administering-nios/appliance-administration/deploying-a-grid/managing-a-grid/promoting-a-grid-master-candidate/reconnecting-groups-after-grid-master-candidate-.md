...
2. In the **GMC Group Promotion Schedule** editor, specify the following:
   ...
   - Click the **+ **icon and specify the following in **Add GMC Group** **Wizard**:
     ...
     - **Time** **Zone**: Select a time zone that applies to the start time you enter. If this time zone is different  
       ...
       (For information about setting the Grid and member time zones, see [*Managing Time Settings*](/nios90/infoblox-nios-9-0-x/administering-nios/appliance-administration/managing-appliance-operations/managing-time-settings)). After the Grid Master Candidate promotion, members will reconnect based on the selected time zone.
...
Use the `set gmc_promotion disable` CLI command to disable the **Activate GMC Group Promotion Schedule **option. Note that, this command can be executed on Grid Master and Grid Master Candidate. For more information see, [*set gmc_promotion*](/nios90/infoblox-nios-9-0-x/using-the-nios-cli/set-commands/set-gmc-promotion)*.*

> **warning**
>
> Notes
> 
> - If the **Activate GMC Group Promotion Schedule** option is not enabled on Grid Master, and if you choose to continue with Grid Master Candidate promotion using the command, `set promote_master`, then the Grid Master Candidate promotion works as described in [*Promoting the Master Candidate*](/nios90/infoblox-nios-9-0-x/administering-nios/appliance-administration/deploying-a-grid/managing-a-grid/promoting-a-grid-master-candidate/promoting-the-master-candidate).
