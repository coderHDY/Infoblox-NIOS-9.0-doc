...
pageId: "280403227"
---
When you disable synchronization with a Microsoft server, the managing Grid member completes any ongoing synchronization and does not start a new one. Synchronization resumes when the Microsoft server is re-enabled. The synchronized DNS data stays in the same state until synchronization resumes. For information, see [*Disabling Synchronization*](/nios90/infoblox-nios-9-0-x/administering-nios/configuring-microsoft-windows-servers/managing-microsoft-windows-servers/managing-microsoft-servers/Managing+Microsoft+Servers#Disabling-Synchronization).  
