...
A limited-access admin group can view and export the DHCP lease history if it has read-only permission to the DHCP lease history. For information on setting permissions for the DHCP lease history, see *[Administrative Permissions for the IPv4 and IPv6 DHCP Lease Histories](/nios90/infoblox-nios-9-0-x/administering-nios/appliance-administration/managing-administrators/administrative-permissions-for-dhcp-resources)*. In addition, you can export the displayed DHCP current lease information or you can export them to a CSV file.  
