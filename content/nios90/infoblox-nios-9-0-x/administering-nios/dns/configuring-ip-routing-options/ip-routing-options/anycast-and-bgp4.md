...
The BGP protocol service is automatically configured to send SNMP queries about BGP runtime data. The appliance sends SNMP traps to its neighboring routers when it encounters issues with the protocol. BGP is configured to send SNMP traps as defined in *RFC4273* *Definitions* *of* *Managed* *Objects* *for* *BGP-4*. You must enable and configure the SNMP trap receiver on the Grid member for the member to send SNMP traps. For information, see [*SNMP MIB Hierarchy*](/nios90/infoblox-nios-9-0-x/administering-nios/monitoring/monitoring-with-snmp/snmp-mib-hierarchy).  
